...
% and numerical analysis of a given circuit object.
    ...
    methods(Static)
        ...
        % Symbolic analysis of given circuit object.
            ...
            % Passives (R, L, C).
            for index = 1:obj.num_passives
                X = obj.Passives(index);
            ...
            for index = 1:obj.num_VCCSs
                ...
                switch pattern
                    case {'0000', '0011',  '0001', '0010', '0100', '1000', '1100'}
                        error('error in VCCS');
                    case '1111'
            ...
            for index = 1:obj.num_CCVSs
                ...
                j{num_vs_parsed} = sprintf('I_%s', CCVS.id);

                cv = CCVS.ctrl_anode;
                cvInd = find(contains(j, cv));
                hInd = find(contains(j, CCVS.id));
                D{hInd, cvInd} = ['-' CCVS.id];
            end
            ...
            for index = 1:obj.num_CCCSs
                ...
                cv = obj.CCCSs(index).ctrl_anode;
                cvInd = find(contains(j, cv));

                if (CCCS.anode ~= 0)
                    B{CCCS.anode, cvInd} = [B{CCCS.anode, cvInd} ' + ' CCCS.id];
                end
                if (CCCS.cathode ~= 0)
                    B{CCCS.cathode, cvInd} = [B{CCCS.cathode, cvInd} ' - ' CCCS.id];
                end
            end
        ...
        % Analyze circuit, using numerical values, if any exist.
            ...
            if ~obj.symbolically_analyzed
                error('Circuit has not been symbolically analyzed.');
            end
        ...
        % Returns symbolic transfer function of circuit, given input and output node.
            ...
            tic;
            if ~obj.symbolically_analyzed
                disp('Symbolically analyzing circuit.');
        ...
        % Returns transfer function object of circuit, given input and output node.
            ...
            if ~obj.numerically_analyzed
                disp('Numerically evaluating circuit.');
    ...
    methods(Static, Access = private)
        ...
        function find_v_and_i(obj)
         % Find voltages and currents in single array of specific element-type.
            ...
            for index = 1:obj.num_passives
                X = obj.Passives(index); from_node = 0; to_node = 0;
